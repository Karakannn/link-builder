...
function isCustomDomain(hostname: string): boolean {
    const appDomain = process.env.NEXT_PUBLIC_APP_DOMAIN || "link-builder-one.vercel.app"; // BURASI DEĞİŞTİ!
...
export default clerkMiddleware(async (auth, req: NextRequest) => {
    ...
    if (
        ...
        console.log("📁 Static file - bypassing");
        return NextResponse.next();
    }
    ...
    if (isCustomDomain(hostname)) {
        ...
        // Custom domain'de sadece root path'e izin ver
        if (url.pathname !== "/") {
            console.log("❌ Custom domain - only root allowed:", url.pathname);
            return new NextResponse(null, { status: 404 });
        }
    ...
    // Ana domain için normal auth flow
    console.log("🏠 Main domain - processing auth");
