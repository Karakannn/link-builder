...
const SettingsTab = () => {
    ...
    const getCurrentStyles = () => {
        ...
        // Ensure margin and padding values are always present
        return {
            ...defaultStyles,
            ...styles,
        };
    ...
    const getGridColumnCount = (): number => {
        const template = getCurrentStyles().gridTemplateColumns as string || '';
        const match = template.match(/repeat\((\d+),/);
        return match ? parseInt(match[1]) : 12;
    };

    const updateGridColumnCount = (count: number) => {
        const validCount = Math.max(1, Math.min(24, count));

        // Update all containers to ensure they don't exceed new column count
        const content = state.editor.selectedElement.content;
        if (Array.isArray(content)) {
            const updatedContent = content.map(container => {
                const currentSpan = parseInt(container.styles.gridColumn?.replace('span ', '') || '6');
                const newSpan = Math.min(currentSpan, validCount);
            ...
            dispatch({
                ...
                payload: {
                    elementDetails: {
                        ...
                        styles: {
                            ...state.editor.selectedElement.styles,
                            gridTemplateColumns: `repeat(${validCount}, 1fr)`,
                        },
                    },
                },
            });
            ...
            handleOnChanges({
                target: {
                    ...
                    value: `repeat(${validCount}, 1fr)`
                }
            });
        }
    };

    const getContainerSpan = (index: number): number => {
        const content = state.editor.selectedElement.content;
        if (Array.isArray(content) && content[index]) {
            const gridColumn = content[index].styles.gridColumn;
            if (typeof gridColumn === 'string' && gridColumn.includes('span')) {
                return parseInt(gridColumn.replace('span ', '')) || 6;
            }
        }
        return 6; // Default to half width
    };

    const updateContainerSpan = (index: number, span: number) => {
        const content = state.editor.selectedElement.content;
        if (!Array.isArray(content) || !content[index]) return;

        // Ensure span is between 1 and 12
        const validSpan = Math.max(1, Math.min(12, span));

        const updatedContent = [...content];
        updatedContent[index] = {
            ...updatedContent[index],
            styles: {
                ...updatedContent[index].styles,
    ...
    const updateContainerCount = (newCount: number) => {
        const currentContainers = Array.isArray(state.editor.selectedElement.content)
            ? state.editor.selectedElement.content
            : [];

        let updatedContainers = [...currentContainers];

        if (newCount > currentContainers.length) {
            const containersToAdd = newCount - currentContainers.length;
            for (let i = 0; i < containersToAdd; i++) {
                const newContainer: EditorElement = {
                    id: v4(),
                    name: `Container ${currentContainers.length + i + 1}`,
                    content: [],
                    styles: {
                        ...defaultStyles,
                      /*   minHeight: "120px", */
                        width: "100%",
                        gridColumn: "span 6", // Default to half width
                    } as React.CSSProperties,
                    type: "container",
                };
                updatedContainers.push(newContainer);
            }
        } else if (newCount < currentContainers.length) {
            updatedContainers = updatedContainers.slice(0, newCount);
        }
        ...
        dispatch({
            ...
            payload: {
                elementDetails: {
                    ...
                    styles: {
                        ...state.editor.selectedElement.styles,
                        gridTemplateColumns: "repeat(12, 1fr)", // Always use 12 column grid
                    },
                },
            },
        });
                ...
                <AccordionContent className="px-6">
                    ...
                    {state.editor.selectedElement.type === "gridLayout" && (
                        <div className="flex flex-col gap-4">
                            {/* Container Count */}
                            <div className="flex flex-col gap-2">
                                <p className="text-muted-foreground">Konteyner Sayısı</p>
                                <p className="text-xs text-muted-foreground">
                                    Grid içinde kaç tane alan olacağını belirleyin
                                </p>
                                <div className="flex items-center gap-2">
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => {
                                            const currentCount = Array.isArray(state.editor.selectedElement.content)
                                                ? state.editor.selectedElement.content.length
                                                : 1;
                                            if (currentCount > 1) {
                                        ...
                                        disabled={Array.isArray(state.editor.selectedElement.content) &&
                                            state.editor.selectedElement.content.length <= 1}
                                    >
                                        <Minus className="h-4 w-4" />
                                    </Button>
                                    ...
                                    <Button
                                        ...
                                        disabled={Array.isArray(state.editor.selectedElement.content) &&
                                            state.editor.selectedElement.content.length >= 24}
                                    >
                                        <Plus className="h-4 w-4" />
                                    </Button>
                                </div>
                            ...
                            <div className="flex flex-col gap-2">
                                <p className="text-muted-foreground">Grid Sütun Sayısı</p>
                                <p className="text-xs text-muted-foreground">
                                    Satırda maksimum kaç sütun olacağını belirleyin. Her konteyner bu sütunlardan istediği kadarını kullanabilir.
                                </p>
                                <div className="flex items-center gap-2">
                                    <Input
                                        type="number"
                                        min="1"
                                        max="24"
                                        value={getGridColumnCount()}
                                        onChange={(e) => updateGridColumnCount(parseInt(e.target.value) || 12)}
                                        className="flex-1"
                                    />
                                    <span className="text-sm text-muted-foreground">sütun</span>
                                </div>
                            </div>

                            {/* Container Widths */}
                            <div className="flex flex-col gap-2">
                                <p className="text-muted-foreground">Konteyner Genişlikleri</p>
                                <p className="text-xs text-muted-foreground">
                                    Her konteynerin kaç sütun kaplayacağını ayarlayın (1 - {getGridColumnCount()} arası)
                                </p>

                                <div className="space-y-3">
                                    {Array.isArray(state.editor.selectedElement.content) &&
                                        state.editor.selectedElement.content.map((item, index) => (
                                            <div key={item.id} className="space-y-2">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-sm">Konteyner {index + 1}</span>
                                                    <span className="text-xs text-muted-foreground">
                                                        {getContainerSpan(index)} / {getGridColumnCount()} sütun
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Slider
                                                        value={[getContainerSpan(index)]}
                                                        onValueChange={(value) => updateContainerSpan(index, value[0])}
                                                        min={1}
                                                        max={getGridColumnCount()}
                                                        step={1}
                                                        className="flex-1"
                                                    />
                                                    <Input
                                                        ...
                                                        max={getGridColumnCount()}
                                                        value={getContainerSpan(index)}
                                                        onChange={(e) => updateContainerSpan(index, parseInt(e.target.value) || 1)}
                                                        className="w-16"
                                                    />
                                                </div>
                                            </div>
                            ...
                            <div className="border rounded p-3 bg-muted/30">
                                ...
                                <div className="space-y-1">
                                    {(() => {
                                        const containers = Array.isArray(state.editor.selectedElement.content)
                                            ? state.editor.selectedElement.content
                                            : [];
                                        ...
                                        containers.forEach((container, index) => {
                                            const span = getContainerSpan(index);

                                            if (currentRowSpan + span > columnCount) {
                                                if (currentRow.length > 0) {
                                                    rows.push(currentRow);
                                                }
                                                currentRow = [{ index, span }];
                                                currentRowSpan = span;
                                            } else {
                                        ...
                                        return rows.map((row, rowIndex) => (
                                            <div key={rowIndex} className="flex gap-[2px]">
                                                {row.map(({ index, span }) => (
                                                    <div
                                                        key={index}
                                                        className="bg-blue-500 h-8 rounded text-white text-xs flex items-center justify-center font-medium"
                                                        style={{
                                                            width: `calc(${(span / columnCount) * 100}% - ${row.length > 1 ? '2px' : '0px'})`
                                                        }}
                                                    >
                                                        {index + 1}
                                                    </div>
                            ...
                            {/* Grid Gap */}
                            <div className="flex flex-col gap-2">
                                <p className="text-muted-foreground">Konteynerler Arası Boşluk</p>
                                <p className="text-xs text-muted-foreground">
                                    Konteynerlerin arasındaki boşluğu piksel veya rem cinsinden girin (örn: 16px veya 1rem)
                                </p>
                                <Input
                                    placeholder="1rem"
                                    ...
                                    onChange={(e) => {
                                        handleOnChanges({
                                            target: { id: "gap", value: e.target.value },
                                        });
                                    }}
                                />
                            </div>
                            ...
                            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                ...
                                <p className="text-xs text-blue-600 dark:text-blue-300 mt-1">
                                    Tablet veya Mobil görünüme geçerek farklı cihazlar için özel düzenlemeler yapabilirsiniz.
                                    Örneğin: Mobilde tüm konteynerler tam genişlik, masaüstünde yan yana olabilir.
                                </p>
                                <p className="text-xs text-blue-600 dark:text-blue-300 mt-1">
                                    Konteynerleri sürükleyip bırakarak sıralarını değiştirebilirsiniz!
                                </p>
                            </div>
                        </div>
                    )}
