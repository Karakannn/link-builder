...
const FunnelEditorNavigation: React.FC<Props> = ({ user, pageDetails }) => {
    ...
    const { changeDevice, toggleLiveMode, togglePreviewMode } = useUIActions();
        ...
        <TooltipProvider>
            ...
            >
                <aside className="flex items-center gap-4 max-w-[260px] w-[300px]">
                    <Link href={isLandingModalPage ? `/admin/landing-modal` : isLiveStreamCardPage ? `/admin/live-stream-cards` : `/admin/sites`}>
                        <ArrowLeftCircle />
                    </Link>
                ...
                <aside className="flex items-center gap-2">
                    ...
                    <Button disabled={!(state.history.currentIndex > 0)} onClick={undo} variant={"ghost"} size={"icon"} className="hover:bg-slate-800">
                        <Undo2 />
                    </Button>
                    <Button
